...
const INIT_ATT = {
  [TODAY]:{S001:"Present",S002:"Absent",S003:"Present",S004:"Late",S005:"Present",S006:"Present"}
};
...
function Btn({children,onClick,variant="primary",size="md",disabled,full,style:s={}}){
  ...
  const v={
    ...
    secondary:{background:"transparent",color:C.primary,borderColor:C.primary},
    danger:{background:C.danger,color:"#fff",borderColor:C.danger},
    ghost:{background:"transparent",color:C.textMd,borderColor:C.border},
...
function Card({children,style:s={},onClick}){
  return <div onClick={onClick} style={{background:C.card,borderRadius:12,border:`1px solid ${C.border}`,boxShadow:C.shadow,...s}}>{children}</div>;
}
...
function PageHdr({title,sub,actions}){
  return <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:18,gap:10,flexWrap:"wrap"}}>
    <div><h2 style={{margin:0,fontSize:18,fontWeight:700,color:C.text}}>{title}</h2>
    {sub&&<p style={{margin:"4px 0 0",fontSize:12,color:C.textMd}}>{sub}</p>}</div>
...
function Table({cols,rows}){
  return <div style={{overflowX:"auto"}}>
    <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
      <thead><tr style={{background:C.primaryDk}}>
        {cols.map(c=><th key={c} style={{padding:"9px 12px",textAlign:"left",fontWeight:600,fontSize:11,color:"#fff",whiteSpace:"nowrap"}}>{c}</th>)}
      </tr></thead>
      <tbody>
        {rows.length===0&&<tr><td colSpan={cols.length} style={{padding:32,textAlign:"center",color:C.textMd}}>No records found.</td></tr>}
        {rows.map((r,i)=><tr key={i} style={{borderBottom:`1px solid ${C.border}`,background:i%2===0?"#fff":"#fafafa"}}
          onMouseEnter={e=>e.currentTarget.style.background=C.primaryLt}
          onMouseLeave={e=>e.currentTarget.style.background=i%2===0?"#fff":"#fafafa"}>
          {r.map((cell,j)=><td key={j} style={{padding:"9px 12px",verticalAlign:"middle"}}>{cell}</td>)}
        </tr>)}
      </tbody>
    </table>
  </div>;
...
// ─── FLOATING AI ──────────────────────────────────────
function FloatingAI({school}){
  const [open,setOpen]=useState(false);
  const [msgs,setMsgs]=useState([{role:"assistant",text:"Assalam-o-Alaikum! Main Paragon School ka AI Assistant hoon.\n\nUrdu ya English mein poochh saktay hain:\n• Admission info\n• Fee structure\n• School timing\n• Subjects & homework help"}]);
  ...
  const send=async()=>{
    ...
    setLoading(true);
    try{
      const res=await fetch("const res = await fetch(   "const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"https://api.anthropic.com/v1/messages"""""""""""""""""""""""""""""""""",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({
          model:"claude-sonnet-4-6",max_tokens:800,
          system:`You are the AI Assistant for ${school.name}, ${school.address}. Phone: ${school.phone}. WhatsApp: ${school.wa}. Email: ${school.email}. Hours: ${school.hours}. Principal: ${school.principal}. Founded: ${school.founded}. Classes Play Group to Class 8. Fee Rs.2000-4000/month. Admissions open June 30 2025. Reply in same language as user (Urdu or English). Be polite, professional, helpful. Never invent facts.`,
          messages:[...msgs,{role:"user",text:q}].slice(-8).map(m=>({role:m.role,content:m.text}))
        })
      });
      const data=await res.json();
      setMsgs(prev=>[...prev,{role:"assistant",text:data.content?.[0]?.text||"Sorry, unable to respond."}]);
    }catch{
      setMsgs(prev=>[...prev,{role:"assistant",text:`Connection error.\nCall us: ${school.phone}\nWhatsApp: ${school.wa}`}]);
    }
    setLoading(false);
  };

  return <>
    <button onClick={()=>setOpen(o=>!o)} style={{position:"fixed",bottom:20,right:20,width:52,height:52,borderRadius:"50%",background:`linear-gradient(135deg,${C.primaryDk},${C.primary})`,border:"none",cursor:"pointer",boxShadow:"0 4px 16px rgba(26,92,56,.5)",zIndex:8000,color:"#fff",fontSize:22}}>
      {open?"✕":"💬"}
    </button>
    {open&&<div style={{position:"fixed",bottom:82,right:20,width:340,height:500,background:"#fff",borderRadius:16,boxShadow:"0 8px 40px rgba(0,0,0,.18)",zIndex:8000,display:"flex",flexDirection:"column",overflow:"hidden",border:`1px solid ${C.border}`}}>
      <div style={{background:`linear-gradient(135deg,${C.primaryDk},${C.primary})`,padding:"11px 14px",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        <div style={{width:30,height:30,borderRadius:"50%",background:"rgba(255,255,255,.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:900,color:"#fff"}}>P</div>
        <div style={{flex:1}}><div style={{fontSize:13,fontWeight:700,color:"#fff"}}>Paragon AI Assistant</div><div style={{fontSize:10,color:"rgba(255,255,255,.6)"}}>Online</div></div>
        <button onClick={()=>setMsgs([{role:"assistant",text:"Chat cleared. How can I help?"}])} style={{background:"none",border:"none",color:"rgba(255,255,255,.6)",cursor:"pointer",fontSize:11}}>Clear</button>
      </div>
      <div style={{flex:1,overflowY:"auto",padding:12,background:"#f8fafc",display:"flex",flexDirection:"column",gap:8}}>
        {msgs.map((m,i)=><div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
          <div style={{maxWidth:"82%",background:m.role==="user"?`linear-gradient(135deg,${C.primary},${C.primaryDk})`:"#fff",color:m.role==="user"?"#fff":C.text,borderRadius:m.role==="user"?"14px 14px 3px 14px":"14px 14px 14px 3px",padding:"10px 13px",fontSize:12,lineHeight:1.65,boxShadow:C.shadow,border:m.role==="user"?"none":`1px solid ${C.border}`,whiteSpace:"pre-wrap"}}>{m.text}</div>
        </div>)}
        {loading&&<div style={{display:"flex",justifyContent:"flex-start"}}>
          <div style={{background:"#fff",borderRadius:"14px 14px 14px 3px",padding:"10px 14px",boxShadow:C.shadow,border:`1px solid ${C.border}`,display:"flex",gap:4}}>
            {[0,1,2].map(d=><div key={d} style={{width:6,height:6,borderRadius:"50%",background:C.primary,opacity:.4+d*.2}}/>)}
          </div>
        </div>}
        <div ref={ref}/>
      </div>
      <div style={{padding:"8px 10px",borderTop:`1px solid ${C.border}`,background:"#fff",display:"flex",gap:8}}>
        ...
        <button onClick={send} disabled={loading||!input.trim()} style={{width:34,height:34,borderRadius:"50%",border:"none",background:loading||!input.trim()?C.border:`linear-gradient(135deg,${C.primary},${C.primaryDk})`,cursor:loading||!input.trim()?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
  ...
  return <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",minHeight:"100%",background:"#fff"}}>
    ...
    {page==="home"&&<>
      ...
      <div style={{padding:"40px 36px",background:"#fff"}}>
        <div style={{maxWidth:800,margin:"0 auto",display:"flex",gap:24,flexWrap:"wrap",alignItems:"flex-start"}}>
          ...
          <div style={{flex:1,minWidth:200}}>
            ...
            <p style={{color:C.textMd,lineHeight:1.9,fontSize:13,margin:"0 0 8px"}}>At {school.name}, we believe true education combines academic excellence with strong Islamic values. Our dedicated team prepares every student for a successful and righteous future.</p>
      ...
      <div style={{...HDR,padding:"36px"}}>
        ...
        <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
          ...
          <a href={`mailto:${school.email}`} style={{background:"rgba(255,255,255,.15)",color:"#fff",padding:"11px 22px",borderRadius:6,textDecoration:"none",fontWeight:700,fontSize:12,border:"1px solid rgba(255,255,255,.3)",textAlign:"center"}}>{school.email}<br/><span style={{fontWeight:400,opacity:.85,fontSize:10}}>Email Us</span></a>
        </div>
      </div>
    ...
    {page==="about"&&<div style={{padding:"44px 36px",maxWidth:860,margin:"0 auto"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:14,marginBottom:22}}>
        {[["Founded",school.founded],["Location","Dera Ismail Khan, KPK"],["Principal",school.principal],["Classes","Play Group – Class 8"],["Students","1,200+"],["Staff","62 Members"]].map(([k,v])=>(
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        {[["Vision","To be a leading Islamic educational institution producing academically excellent, morally upright citizens."],["Mission","To provide quality education rooted in Islamic values, developing students intellectually, spiritually and physically."]].map(([t,d])=>(
          <Card key={t} style={{padding:18}}><h3 style={{color:C.primary,margin:"0 0 8px",fontSize:13,fontWeight:700}}>{t}</h3><p style={{color:C.textMd,lineHeight:1.8,fontSize:12,margin:0}}>{d}</p></Card>
        ))}
      </div>
    </div>}
        ...
        {[[school.phone,"Mobile",`tel:${school.phone}`],[school.wa,"WhatsApp",`https://wa.me/${school.wa.replace(/\D/g,"")}`],[school.email,"Email",`mailto:${school.email}`],[school.address,"Address","#"]].map(([val,label,href])=>(
          <a key={label} href={href} target={href.startsWith("http")?"_blank":"_self"} rel="noopener" style={{textDecoration:"none"}}>
            <Card style={{padding:14,textAlign:"center"}}>
              <div style={{fontSize:10,fontWeight:700,color:C.textMd,textTransform:"uppercase",marginBottom:4}}>{label}</div>
              <div style={{fontSize:11,fontWeight:600,color:C.primary,lineHeight:1.5}}>{val}</div>
            </Card>
          </a>
        ))}
      </div>
...
function Dashboard({students,fees,attendance}){
  ...
  return <div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(155px,1fr))",gap:8}}>
      {CLASSES.map(cls=>{
        const cnt=students.filter(s=>s.cls===cls).length;
        return <Card key={cls} style={{padding:"11px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><div style={{fontWeight:700,color:C.text,fontSize:12}}>{cls}</div><div style={{fontSize:10,color:C.textMd}}>{cnt} students</div></div>
          <div style={{width:30,height:30,borderRadius:"50%",background:C.primaryLt,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,color:C.primary,fontSize:12}}>{cnt}</div>
        </Card>;
      })}
    </div>
  </div>;
...
function Students({students,setStudents}){
  ...
  const filtered=useMemo(()=>students.filter(s=>
    ...
    (!search||(s.name+s.father+s.id).toLowerCase().includes(search.toLowerCase()))
  ),[students,filterCls,search]);
  ...
  const save=()=>{
    ...
    const clsN=form.cls.replace(/\D/g,"").padStart(2,"0")||"PG";
    const roll=`${clsN}${form.section}-${String(cnt).padStart(2,"0")}`;
      ...
      <Table cols={["ID","Roll","Name","Father","Class","Sec","Contact","Status","Action"]}
        rows={filtered.map(s=>[
          ...
          <div style={{display:"flex",gap:4}}>
            <Btn size="sm" onClick={e=>{e.stopPropagation();setSel(s);}}>View</Btn>
            <Btn size="sm" variant="danger" onClick={e=>{e.stopPropagation();setStudents(p=>p.filter(x=>x.id!==s.id));}}>Del</Btn>
          </div>
        ])}/>
    </Card>
...
function Attendance({students,attendance,setAttendance}){
  ...
  const rate=clsStu.length?Math.round(stats.present/clsStu.length*100):0;
  ...
  return <div>
    ...
    <Card style={{padding:"12px 14px",marginBottom:12}}>
      <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
        <div style={{flex:1,minWidth:120}}>
          <label style={{fontSize:10,fontWeight:700,color:C.textMd,textTransform:"uppercase",display:"block",marginBottom:4}}>Class</label>
          <select value={cls} onChange={e=>setCls(e.target.value)} style={{width:"100%",padding:"7px 10px",border:`1px solid ${C.border}`,borderRadius:6,fontSize:12,fontFamily:"inherit"}}>{CLASSES.map(c=><option key={c}>{c}</option>)}</select>
        </div>
        <div style={{flex:1,minWidth:100}}>
          <label style={{fontSize:10,fontWeight:700,color:C.textMd,textTransform:"uppercase",display:"block",marginBottom:4}}>Section</label>
          <select value={sec} onChange={e=>setSec(e.target.value)} style={{width:"100%",padding:"7px 10px",border:`1px solid ${C.border}`,borderRadius:6,fontSize:12,fontFamily:"inherit"}}>{["A","B","C"].map(s=><option key={s}>{s}</option>)}</select>
        </div>
        <div style={{flex:1,minWidth:120}}>
          <label style={{fontSize:10,fontWeight:700,color:C.textMd,textTransform:"uppercase",display:"block",marginBottom:4}}>Date</label>
          <input type="date" value={date} onChange={e=>setDate(e.target.value)} style={{width:"100%",padding:"7px 10px",border:`1px solid ${C.border}`,borderRadius:6,fontSize:12,fontFamily:"inherit"}}/>
        </div>
    ...
    <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:12}}>
      <StatCard label="Present" value={stats.present} color={C.success} sub={`${rate}%`}/>
    ...
    {tab==="mark"&&<Card style={{overflow:"hidden"}}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
        ...
        <tbody>{clsStu.map((s,i)=>{const st=dayAtt[s.id]||"";return <tr key={s.id} style={{borderBottom:`1px solid ${C.border}`,background:i%2===0?"#fff":"#fafafa"}}>
          ...
          <td style={{padding:"6px 12px"}}><div style={{display:"flex",gap:5}}>
            {["Present","Absent","Late","Leave"].map(status=><button key={status} onClick={()=>mark(s.id,status)} style={{padding:"3px 8px",borderRadius:5,border:`1.5px solid ${st===status?(status==="Present"?C.success:status==="Absent"?C.danger:status==="Late"?C.warn:C.info):C.border}`,cursor:"pointer",fontSize:10,fontWeight:700,background:st===status?(status==="Present"?C.success:status==="Absent"?C.danger:status==="Late"?C.warn:C.info):"transparent",color:st===status?"#fff":C.textMd,fontFamily:"inherit"}}>{status}</button>)}
          </div></td>
        </tr>;})}
        </tbody>
      </table>}
...
function Fees({fees,setFees}){
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:6,marginBottom:12}}>
      {["All","Paid","Unpaid"].map(f=><Btn key={f} variant={filter===f?"primary":"ghost"} size="sm" onClick={()=>setFilter(f)}>{f}</Btn>)}
    </div>
    <Card style={{overflow:"hidden"}}>
...
function Exams(){
  ...
  return <div>
    <PageHdr title="Exams & Results" sub="Mid-Term 2025" actions={<Btn>+ Add Result</Btn>}/>
    <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:16}}>
    ...
    <Card style={{overflow:"hidden"}}>
      <Table cols={["Student","Class","Math","English","Science","Urdu","Islamiat","Total/500","%","Grade"]}
...
function Timetable(){
  ...
  return <div>
    <PageHdr title="Class Timetable" actions={
      <select value={cls} onChange={e=>setCls(e.target.value)} style={{padding:"7px 12px",border:`1px solid ${C.border}`,borderRadius:6,fontSize:12,fontFamily:"inherit"}}>
        {CLASSES.map(c=><option key={c}>{c}</option>)}
      </select>}/>
    <Card style={{overflow:"auto"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:600}}>
        <thead><tr style={{background:C.primaryDk,color:"#fff"}}>
          <th style={{padding:"9px 12px",textAlign:"left",fontWeight:600,minWidth:100}}>Period</th>
          {days.map(d=><th key={d} style={{padding:"9px 12px",textAlign:"center",fontWeight:600}}>{d}</th>)}
        </tr></thead>
        <tbody>{periods.map((p,pi)=>(
          <tr key={pi} style={{borderBottom:`1px solid ${C.border}`,background:p.l==="Break"?"#fffde7":"transparent"}}>
            <td style={{padding:"8px 12px",background:C.bg}}>
              <div style={{fontWeight:700,color:C.text,fontSize:11}}>{p.l}</div>
              <div style={{fontSize:10,color:C.textMd}}>{p.t}</div>
            </td>
            {days.map((d,di)=>{
              ...
              return <td key={d} style={{padding:"5px 7px"}}>
                <div style={{background:colors[si],color:tColors[si],borderRadius:6,padding:"4px 7px",textAlign:"center",fontSize:10,fontWeight:700}}>{subjects[si]}</div>
              </td>;
            })}
          </tr>
        ))}</tbody>
      ...
      <div>
        <Card style={{padding:32,border:`5px double ${C.gold}`,borderRadius:4,fontFamily:"Georgia,serif"}}>
          ...
          <div style={{lineHeight:2.1,fontSize:12.5,color:C.text}}>
            ...
            {type==="character"&&<p>The student maintained <strong>excellent character, discipline and Islamic conduct</strong> throughout their stay at this institution.</p>}
            {type==="leaving"&&<p>The student is leaving this institution effective <strong>{date}</strong>. We wish them every success in their future endeavors.</p>}
            {type==="bonafide"&&<p>This certificate is issued on request to confirm that the above-named student is a bonafide student of this institution.</p>}
          </div>
...
const NAV=[
  ...
  {group:"Administration",items:[{id:"dashboard",label:"Admin Dashboard"},{id:"students",label:"Students"},{id:"attendance",label:"Attendance"},{id:"fees",label:"Fee Management"},{id:"exams",label:"Exams & Results"},{id:"teachers",label:"Teachers"}]},
...
// ─── AI PAGE ──────────────────────────────────────────
function AIPage(){
  const [msgs,setMsgs]=useState([{role:"assistant",text:"Assalam-o-Alaikum!\n\nMain Paragon School ka AI Assistant hoon.\nUrdu ya English mein poochh saktay hain:\n• Admission info\n• Fee structure\n• Homework help\n• General questions"}]);
  const [input,setInput]=useState("");
  const [loading,setLoading]=useState(false);
  const ref=useRef(null);
  useEffect(()=>{ref.current?.scrollIntoView({behavior:"smooth"});},[msgs]);
  ...
  const send=async()=>{
    ...
    setInput("");setMsgs(prev=>[...prev,{role:"user",text:q}]);setLoading(true);
    try{
      const res=await fetch("const res = await fetch(   "const res = await fetch(   "const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"const res = await fetch(   const send = async () => {   const q = input.trim();   if (!q || loading) return;   setInput("");   setMsgs(prev => [     ...prev,      {role:"user", text:q}   ]);   setLoading(true);      // Simulate AI response   setTimeout(() => {     const replies = {       "admission": "Admission ke liye call karein: +92 37 5378045 ya WhatsApp: +92 345 2247258. Last date June 30, 2025.",       "fee": "Fee structure: Play Group Rs.2000, Class 1-5 Rs.3000, Class 6-8 Rs.3800 per month.",       "timing": "School timing: Monday to Saturday, 8:00 AM to 2:00 PM.",       "contact": "Phone: +92 37 5378045, WhatsApp: +92 345 2247258, Email: info@paragonislamicschool.edu.pk",     };          const qLower = q.toLowerCase();     let reply = "Shukriya aapke sawaal ke liye! Mazeed maloomat ke liye call karein: +92 37 5378045";          if(qLower.includes("admission") || qLower.includes("داخلہ"))        reply = replies.admission;     else if(qLower.includes("fee") || qLower.includes("فیس"))        reply = replies.fee;     else if(qLower.includes("time") || qLower.includes("timing") || qLower.includes("وقت"))        reply = replies.timing;     else if(qLower.includes("contact") || qLower.includes("phone") || qLower.includes("رابطہ"))        reply = replies.contact;            setMsgs(prev => [       ...prev,        {role:"assistant", text:reply}     ]);     setLoading(false);   }, 1000); };"https://api.anthropic.com/v1/messages""""""""""""""""""""""""""""""""""",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:800,system:`You are the AI Assistant for ${SCHOOL.name}, ${SCHOOL.address}. Phone: ${SCHOOL.phone}. WhatsApp: ${SCHOOL.wa}. Email: ${SCHOOL.email}. Hours: ${SCHOOL.hours}. Principal: ${SCHOOL.principal}. Classes Play Group to Class 8. Fee Rs.2000-4000/month. Reply in same language as user. Be polite and helpful.`,messages:[...msgs,{role:"user",text:q}].slice(-8).map(m=>({role:m.role,content:m.text}))})});
      const data=await res.json();
      setMsgs(prev=>[...prev,{role:"assistant",text:data.content?.[0]?.text||"Sorry, unable to respond."}]);
  ...
  const QUICK=["Admission requirements?","Fee structure kya hai?","School timing?","Help me with Math","Study plan banao"];

  return <div>
    <PageHdr title="AI Assistant" sub="Bilingual — Urdu + English — Powered by Claude AI"/>
    <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
      {QUICK.map((q,i)=><button key={i} onClick={()=>setInput(q)} style={{padding:"5px 12px",borderRadius:20,border:`1px solid ${C.border}`,background:"#fff",cursor:"pointer",fontSize:11,color:C.textMd,fontFamily:"inherit"}}>{q}</button>)}
    </div>
    <Card style={{overflow:"hidden"}}>
      <div style={{background:`linear-gradient(135deg,${C.primaryDk},${C.primary})`,padding:"12px 16px",display:"flex",alignItems:"center",gap:10}}>
        ...
        <div><div style={{fontSize:13,fontWeight:700,color:"#fff"}}>Paragon AI Assistant</div><div style={{fontSize:10,color:"rgba(255,255,255,.6)"}}>پیراگون AI اسسٹنٹ</div></div>
      ...
      <div style={{height:380,overflowY:"auto",padding:16,background:"#f8fafc",display:"flex",flexDirection:"column",gap:10}}>
        {msgs.map((m,i)=><div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
          <div style={{maxWidth:"80%",background:m.role==="user"?`linear-gradient(135deg,${C.primary},${C.primaryDk})`:"#fff",color:m.role==="user"?"#fff":C.text,borderRadius:m.role==="user"?"14px 14px 3px 14px":"14px 14px 14px 3px",padding:"11px 14px",fontSize:13,lineHeight:1.7,boxShadow:C.shadow,border:m.role==="user"?"none":`1px solid ${C.border}`,whiteSpace:"pre-wrap"}}>{m.text}</div>
        </div>)}
        {loading&&<div style={{display:"flex",justifyContent:"flex-start"}}>
          <div style={{background:"#fff",borderRadius:"14px 14px 14px 3px",padding:"11px 16px",boxShadow:C.shadow,border:`1px solid ${C.border}`,display:"flex",gap:4}}>
            {[0,1,2].map(d=><div key={d} style={{width:7,height:7,borderRadius:"50%",background:C.primary,opacity:.4+d*.2}}/>)}
          </div>
        </div>}
        <div ref={ref}/>
      </div>
...
export default function App(){
  ...
  const render=()=>{
    if(active==="website")    return <Website school={SCHOOL}/>;
  ...
  return <div style={{display:"flex",height:"100vh",fontFamily:"'Segoe UI',system-ui,sans-serif",background:C.bg,overflow:"hidden"}}>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:active==="website"?0:18}}>
        {render()}
      </div>
    </div>

    <FloatingAI school={SCHOOL}/>
  </div>;
}
